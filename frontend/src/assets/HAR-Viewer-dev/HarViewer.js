...
function HarViewer(id,additionnalIndicators){
        ...
        for (var i = 0; i < this.showedEntries.length; i++) {
            ...
            for (var j = 0; j < this.listFilters.length; j++) {
                ...
                if (this.listFilters[j] == 'URL') {
                    field.innerHTML = '<a href="'+entry[this.listFilters[j]]+'" title="'+entry[this.listFilters[j]]+'" class="HarViewerTooltip" >'+entry[this.listFilters[j]]+'</a>';
                } else {
                */
                    field.innerHTML = '<p title="'+entry[this.listFilters[j]]+'" class="HarViewerTooltip">'+entry[this.listFilters[j]]+'</p>';
    ...
    this.initResponse = function(){
        ...
        // Content:
        var li = document.createElement('li');
        li.innerHTML = '<li><p><strong>Content:</strong> : '+entryComplete.response.content.text+'</p>';
        $('#HarViewerDetailsResponseList').append(li);
